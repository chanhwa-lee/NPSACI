...
DGP <- function(N, type = "data", taus = NULL, thetas = NULL, r = NULL){
  ...
  if(type == "estimands"){
    ...
    a.rep = H.sample(max(r,10000), X, N, H.true)
    ...
    w.over.H.F = function(a){
      ...
      w.over.H.a.thetas = w.over.H(a, X, N, thetas, H.true, P.A_bar.thetas)
...
#' @output
H.sample = function(r, X, N, H.fit){
  
  # r random binary vectors from A(N) with prob dist'n H(.)
  # a.q \sim H(.) <=> b.q \sim N(0, sigma^2), a.j.q|b.q \sim Ber(f(X.j) + b.q)
  # (N x r) matrix, q-th column = a.q
