...
result_combined = 
  inner_join(result.table.nonpara %>% filter(tau == 0.3, theta %in% c(0,0.3,0.6)),
             result.table.nonpara %>% filter(tau == 0.5, theta %in% c(0,0.3,0.6)),
             by = c("estimand", "theta"),
             suffix = c(".1", ".2"))
...
write.csv(result_combined_multiplied,
          paste0("TableS2.simulation_TPB_Acorr_m",m,"_r",r,"_taus_multiplied.csv"))
