...
N.dist <- function(m) sample(5:20, size = m, replace = TRUE)
...
## A model ##
Amodel = function(X, N, sigma.b, type = "data"){
...
genCluster <- function(N, sigma.b){
  
  ## Step1. Covariate generation
  X = Xmodel(N)
  
  ## Step2. Treatment model
  A = Amodel(X,N, sigma.b)
  fx = Amodel(X,N, sigma.b, type = "fx")
  sigma.b = Amodel(X,N, sigma.b, type = "sigma.b")
...
computeEstimands <- function(N, policy, taus, thetas, r, sigma.b){
...
data.sim = function(m, sigma.b){
  N.list = N.dist(m)
  data = lapply(N.list, genCluster, sigma.b = sigma.b)
...
estimands.sim = function(M, policy, taus, thetas, theta0, r, sigma.b){
  N.list = N.dist(M)
  estimands = lapply(N.list, computeEstimands, policy = policy, taus = taus, thetas = thetas, r = r, sigma.b = sigma.b)
