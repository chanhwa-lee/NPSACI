...
source(glue("{code.dir}/help_TPB.R"))



###----------- Data generating process (DGP) function ---------------###

#' Data simulation and estimands computation based on N, X, A, T, C models
#'
...
## C model ##
Cmodel = function(A,X,N, type = "data", censoring_factor = 0){
  
  ### Generate C from Gamma distribution – to break proportional hazards assumption
  
  ## censoring_factor controls target censoring rate in simulated data (larger factor, smaller C, more censoring)
  shape_C <- function(A, X, N){
    g.A <- (sum(A)-A)/(N-1)
    with(X, (1-censoring_factor)*(0.2 + 0.5*A + 0.5*g.A*X2^2 + 0.1*pmax(X1,0.1) + 0.1*I(Xc2 < 0.5)))
  } 
...
genCluster <- function(N, censoring_factor){
  ...
  T_ = Tmodel(A,X,N)
  C  = Cmodel(A,X,N, censoring_factor = censoring_factor)
...
data.sim = function(m, censoring_factor){
  N.list = N.dist(m)
  data = lapply(N.list, genCluster, censoring_factor = censoring_factor)
...
estimands.sim = function(M, policy, taus, thetas, theta0, r){
  N.list = N.dist(M)
...
### Test ###
# data = data.sim(1000, censoring_factor = 0.0)
# summary(data)
...
# estimands.TypeB = estimands.sim(M = 10, policy = "TypeB", taus = c(0.2,0.4), thetas = c(0.3,0.45,0.6), theta0 = 0.45, r = 100)
