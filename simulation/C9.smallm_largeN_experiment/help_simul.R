...
source(glue("{code.dir}/help_nuis_est.R"))
source(glue("{code.dir}/help_TypeB.R"))

###------ Cluster size (N) distribution ------###
# Always return vector of N's (same cluster size)

N.dist <- function(m, N) rep(N, m)
...
# estimands.sim = function(M, policy, taus, thetas, theta0, r, N){
#   N.list = N.dist(M, N)
...
estimands.sim = function(M, policy, taus, thetas, theta0, r, N, parallel_computing = TRUE) {
  
  # Determine operation type based on the parallel_computing flag
  ...
  # Each iteration of foreach handles one cluster from N.list
  estimands_list = foreach(i = 1:M, .packages = c("dplyr", "tidyr", "stats")) %op% {
    computeEstimands(
      N      = N.list[[i]],
      policy = policy, 
      taus   = taus, 
  ...
  # --- Aggregate Results ---
  estimands = dplyr::bind_rows(estimands_list) %>% 
    group_by(theta, tau) %>% 
    summarise(mu = mean(mu),
              mu_1 = mean(mu_1),
              mu_0 = mean(mu_0),
  ...
  # Final tidy data transformation
  estimands = estimands %>% 
    mutate(de   = mu_1 - mu_0, 
           se_1 = mu_1 - standard$mu_1,
           se_0 = mu_0 - standard$mu_0,
...
### Test ###
# data = data.sim(1000, 5)
# summary(data)

# estimands.TypeB = estimands.sim(M = 10, policy = "TypeB", taus = c(0.2,0.4), thetas = c(0.3,0.45,0.6), theta0 = 0.45, r = 100, N = 500)
# estimands.TypeB %>% dplyr::filter(estimand %in% c("mu", "mu_1", "mu_0")) %>% arrange(tau, theta, estimand) %>% print(n=100)