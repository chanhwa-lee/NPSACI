...
option_list = list(
  ...
  make_option(c("-N", "--N"), action = "store", default = NA, type = "integer",
              help = paste0("Cluster size (fixed)"))
)
...
###----------- Compute estimands  ---------------###
estimands = estimands.sim(M, "TypeB", taus, thetas, theta0, r, N)
print(estimands)
...
saveRDS(estimands, file = glue("estimand/N{N}/estimand_id{task_ID}.rds"))
