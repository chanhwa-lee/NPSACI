...
G.train = function(data_train, X.C.names){
  ...
  ## Fit Random Survival Forest (SRC) regression
  G.fit <- rfsrc(Surv(Y,1-D) ~ ., 
...
S.i_Y.i = function(Y.i, D.i, A.i, X.i, N.i, F.fit){
  ...
  S.pred = predict(F.fit, cbind(data.frame(A = A.i) %>% 
                                  mutate(g.A = (N.i==1)*1 + (N.i!=1)*(sum(A) - A)/(N.i-1)), X.i))
  
  ### Training times for S.fit
  S.times = S.pred$time.interest
  
  ## S.i.S.times: (N.i x n_S.train) matrix where (j,k)-element is S_ij(S.times[k]|A_i, X_i, N_i)
  S.i.S.times = S.pred$survival
...
martingale = function(Y.i, D.i, A.i, X.i, N.i, taus, F.fit, G.fit){
  ...
  ### Prediction over new data from F.fit
  S.pred = predict(F.fit, cbind(data.frame(A = A.i) %>% mutate(g.A = (N.i==1)*1 + (N.i!=1)*(sum(A) - A)/(N.i-1)), X.i))
  
  ### Training times for F.fit
  S.times = S.pred$time.interest
  
  ## (N.i x n_S.train) matrix where (j,k)-element is S_ij(S.times[k]|A_i, X_i, N_i)
  S.i.S.times = S.pred$survival
  ...
  ## (N.i x n_G.train) matrix where (j,k)-element is G_ij(G.times[k]|A_i, X_i, N_i)
  G.i.G.times = G.pred$survival
...
H.train = function(data_train, X.A.names){
  ...
  H.fit = rbart_vi(A ~ . -id, data_train %>% select(c("id", "A", X.A.names)), group.by = id,
