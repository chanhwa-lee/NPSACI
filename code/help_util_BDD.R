...
estimator <- function(data, X.T.names, X.C.names, X.A.names, taus, thetas, theta0, K, r = 0){
  ...
  for (split in 1:K) {
    ...
    for(i in eval.idx){
      ...
      ifvals.mu  [[i]] <-
        OR[[i]]$mu       +
        (m_k/IPCW_BC_weight$mu) * IPCW_BC[[i]]$mu  +
        (m_k/AUG_weight$mu)     * AUG[[i]]$mu

      ## IF for mu_1 at cluster i over thetas
      ## ifvals.mu_1[[i]]: [length(thetas)] x[length(taus)] matrix
      ifvals.mu_1[[i]] <-
        OR[[i]]$mu_1      +
        (m_k/IPCW_BC_weight$mu_1) * IPCW_BC[[i]]$mu_1 +
        (m_k/AUG_weight$mu_1)     * AUG[[i]]$mu_1

      ## IF for mu_0 at cluster i over thetas
      ## ifvals.mu_0[[i]]: [length(thetas)] x[length(taus)] matrix
      ifvals.mu_0[[i]] <-
        OR[[i]]$mu_0      +
        (m_k/IPCW_BC_weight$mu_0) * IPCW_BC[[i]]$mu_0 +
        (m_k/AUG_weight$mu_0)     * AUG[[i]]$mu_0
